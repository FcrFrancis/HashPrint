        ...
        public static bool PrintCommand(string cmd)
        {
            lock (SyncRoot)
            {
                ...
                try
                {
                    ...
                    {
                        case DeviceType.COM:
                            result = comPrint(Encoding.UTF8.GetBytes(cmd));
                            break;
                        case DeviceType.LPT:
                            result = lptPrint(Encoding.UTF8.GetBytes(cmd));
                            break;
                        case DeviceType.DRV:
                            result = drvPrint(Encoding.UTF8.GetBytes(cmd));
                            break;
                        case DeviceType.TCP:
                            result = tcpPrint(Encoding.UTF8.GetBytes(cmd));
                            break;
                    }
